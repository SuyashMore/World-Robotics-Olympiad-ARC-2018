...
void nav_Complete_1_Row(botData& newSensor,botData& oldSensor,Motor& motor)
{
	...
	{
		cout<<"Navigation Step:"<<5<<endl;
		if(stack_the_Block_from_MainJunction_at_hx(SIDE_2,newSensor,oldSensor,motor))
		{
			state.currentStepIndex++;
		}
	...
	{
		cout<<"Navigation Step:"<<9<<endl;
		if(stack_the_Block_from_MainJunction_at_hx(SIDE_5,newSensor,oldSensor,motor))
		{
			state.currentStepIndex++;
		}
	}
